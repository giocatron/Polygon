...
def test_calculate_checkout_line_total(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "polygon.plugins.avatax.plugin.get_cached_tax_codes_or_fetch",
        lambda _: {"PC040156": "desc"},
    )
    manager = get_plugins_manager(plugins=["polygon.plugins.avatax.plugin.AvataxPlugin"])
...
def test_calculate_checkout_total(
    ...
    monkeypatch.setattr(
        "polygon.plugins.avatax.plugin.AvataxPlugin._skip_plugin", lambda *_: False
    )
    manager = get_plugins_manager(plugins=["polygon.plugins.avatax.plugin.AvataxPlugin"])
    checkout_with_item.shipping_address = address
    checkout_with_item.save()
...
def test_get_cached_tax_codes_or_fetch(monkeypatch, settings):
    monkeypatch.setattr("polygon.plugins.avatax.cache.get", lambda x, y: {})
...
def test_taxes_need_new_fetch(monkeypatch, checkout_with_item, address):
    monkeypatch.setattr("polygon.plugins.avatax.cache.get", lambda x: None)
    checkout_with_item.shipping_address = address
    config = AvataxConfiguration(
        username_or_account="wrong_data", password_or_license="wrong_data"
    )
...
def test_taxes_need_new_fetch_uses_cached_data(
    ...
    checkout_data = generate_request_data_from_checkout(checkout_with_item, config)
    monkeypatch.setattr(
        "polygon.plugins.avatax.cache.get", lambda x: [checkout_data, None]
    )
    assert not taxes_need_new_fetch(checkout_data, str(checkout_with_item.token))


def test_get_plugin_configuration(settings):
    settings.PLUGINS = ["polygon.plugins.avatax.plugin.AvataxPlugin"]
...
@patch("polygon.plugins.avatax.plugin.api_get_request")
...
@patch("polygon.plugins.avatax.plugin.api_post_request_task.delay")
...
def test_api_get_request_handles_request_errors(product, monkeypatch):
    ...
    monkeypatch.setattr("polygon.plugins.avatax.requests.get", mocked_response)
...
def test_api_post_request_handles_request_errors(product, monkeypatch):
    mocked_response = Mock(side_effect=RequestException())
    monkeypatch.setattr("polygon.plugins.avatax.requests.post", mocked_response)
...
@patch("polygon.plugins.avatax.get_order_request_data")
@patch("polygon.plugins.avatax.get_cached_response_or_fetch")
