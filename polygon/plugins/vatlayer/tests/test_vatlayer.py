...
def test_vatlayer_plugin_caches_taxes(vatlayer, monkeypatch, product, address):
    ...
    monkeypatch.setattr(
        "polygon.plugins.vatlayer.plugin.get_taxes_for_country", mocked_taxes
    )
...
def test_calculate_checkout_total(
    ...
    manager = get_plugins_manager(
        plugins=["polygon.plugins.vatlayer.plugin.VatlayerPlugin"]
    )
...
def test_get_tax_rate_type_choices(vatlayer, settings, monkeypatch):
    ...
    monkeypatch.setattr(
        "polygon.plugins.vatlayer.plugin.get_tax_rate_types", lambda: expected_choices,
    )
    settings.PLUGINS = ["polygon.plugins.vatlayer.plugin.VatlayerPlugin"]
    manager = get_plugins_manager()
