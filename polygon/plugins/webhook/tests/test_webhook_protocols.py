...
def test_trigger_webhooks_with_aws_sqs(
    ...
    monkeypatch.setattr(
        "polygon.plugins.webhook.tasks.boto3.client", mocked_client_constructor,
    )
...
def test_trigger_webhooks_with_google_pub_sub(
    ...
    monkeypatch.setattr(
        "polygon.plugins.webhook.tasks.pubsub_v1.PublisherClient",
        lambda: mocked_publisher,
    )
    webhook.app.permissions.add(permission_manage_orders)
    webhook.target_url = "gcpubsub://cloud.google.com/projects/polygon/topics/test"
    webhook.save()
...
@pytest.mark.vcr
@patch("polygon.plugins.webhook.tasks.requests.post", wraps=requests.post)
