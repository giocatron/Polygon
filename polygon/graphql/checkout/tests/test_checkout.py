...
@mock.patch("polygon.plugins.webhook.plugin.trigger_webhooks_for_event.delay")
def test_checkout_create_triggers_webhooks(
    ...
    settings.PLUGINS = ["polygon.plugins.webhook.plugin.WebhookPlugin"]
...
@mock.patch(
    "polygon.graphql.checkout.mutations.update_checkout_shipping_method_if_invalid",
...
def mock_get_manager(mocker, fake_manager):
    manager = mocker.patch(
        "polygon.payment.gateway.get_plugins_manager",
        autospec=True,
...
@patch("polygon.graphql.checkout.mutations.clean_shipping_method")
...
def test_clean_checkout_no_shipping_method(checkout_with_item, address):
    ...
    with pytest.raises(ValidationError) as e:
        clean_checkout_shipping(checkout, list(checkout), None, CheckoutErrorCode)
