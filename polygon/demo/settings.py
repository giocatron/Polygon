...
variables are set (see the `polygon.core.utils.random_data.configure_braintree` function
for more details)
...
ROOT_URLCONF = "polygon.demo.urls"

PLUGINS += ["polygon.plugins.anonymize.plugin.AnonymizePlugin"]

GRAPHENE["MIDDLEWARE"].insert(0, "polygon.graphql.middleware.ReadOnlyMiddleware")  # type: ignore

BRAINTREE_API_KEY = os.environ.get("BRAINTREE_API_KEY", "DUMMY_BRAINTRE_KEY")
BRAINTREE_MERCHANT_ID = os.environ.get("BRAINTREE_MERCHANT_ID", "DUMMY_BRAINTRE_MERCH_ID")
BRAINTREE_SECRET_API_KEY = os.environ.get("BRAINTREE_SECRET_API_KEY", "DUMMY_BRAINTRE_SECRET_KEY")
...
PWA_ORIGINS = get_list(os.environ.get("PWA_ORIGINS", "pwa.polygon.io"))
PWA_DASHBOARD_URL_RE = re.compile("^https?://[^/]+/dashboard/.*")

ROOT_EMAIL = os.environ.get("ROOT_EMAIL")

# Remove "polygon.core" and add it after adding "polygon.demo", to have "populatedb"
# command overriden when using demo settings
# (see polygon.demo.management.commands.populatedb).
INSTALLED_APPS.remove("polygon.core")
INSTALLED_APPS += ["polygon.demo", "polygon.core"]
...
def before_send(event: dict, _hint: dict):
    ...
    if ev_logger_name != "polygon.graphql.errors.handled":
        return event
